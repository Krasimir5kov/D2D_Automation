# DOM & Accessibility — Sales Actions: FTTH List (Filters)

## Source
- `sales-actions/ftth-list/default (filters state)/accessibility-report.json`
- `sales-actions/ftth-list/default (filters state)/sanitized-dom.html`
- `sales-actions/ftth-list/default (filters state)/IMPROVEMENTS.md`

## Accessibility Tree

### Violations

- **aria-command-name** — serious, 88 node(s): Ensure every ARIA button, link and menuitem has an accessible name
- **aria-required-parent** — critical, 5680 node(s): Ensure elements with an ARIA role that require parent roles are contained by them
- **button-name** — critical, 1 node(s): Ensure buttons have discernible text
- **color-contrast** — serious, 98 node(s): Ensure the contrast between foreground and background colors meets WCAG 2 AA minimum contrast ratio thresholds
...
# UI Improvements - sales-actions / ftth-list / default

- Capture type: configured route sweep
- Total interactive elements: 5916
...
- aria-command-name: serious; nodes: 88
