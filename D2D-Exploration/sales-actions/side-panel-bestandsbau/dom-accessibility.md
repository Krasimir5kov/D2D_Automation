# DOM & Accessibility — Sales Actions: Bestandsbau Side Panel

## Source
- `sales-actions/bestandsbau-detail-door-level/side-panel-open/accessibility-report.json`
- `sales-actions/bestandsbau-detail-door-level/side-panel-open/sanitized-dom.html`
- `sales-actions/bestandsbau-detail-door-level/side-panel-open/IMPROVEMENTS.md`

## Accessibility Tree

### Violations

- **aria-command-name** — serious, 81 node(s): Ensure every ARIA button, link and menuitem has an accessible name
- **aria-required-parent** — critical, 6804 node(s): Ensure elements with an ARIA role that require parent roles are contained by them
- **button-name** — critical, 1 node(s): Ensure buttons have discernible text
- **color-contrast** — serious, 74 node(s): Ensure the contrast between foreground and background colors meets WCAG 2 AA minimum contrast ratio thresholds
...
# UI Improvements - sales-actions / bestandsbau-detail-door-level / side-panel-open

- Capture type: configured route sweep
- Total interactive elements: 7039
...
- aria-command-name: serious; nodes: 81
- aria-required-parent: critical; nodes: 6804
- button-name: critical; nodes: 1
- color-contrast: serious; nodes: 74
...
- row action button: Icon-only control has no meaningful accessible name. Count: 55; priority: High
- icon-only button: Icon-only control has no meaningful accessible name. Count: 26; priority: High
...
- standard button: Diagnostic DOM path is long and would be brittle as a locator. Count: 4; priority: Medium
...
- row action button: Generic div behaves like a button; prefer a native button. Count: 3; priority: High
...
- dropdown option: Custom dropdown-like component should expose stable roles, names, and expanded/selected state. Count: 6802; priority: Medium
